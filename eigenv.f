      subroutine eigenv(Mn,nres,func)  
      implicit none
c       program zndrv1
c
...
c     %-----------------------------%
c
      integer           maxn, maxnev, maxncv, ldv,i,j,k,nres
      include 'common2.f'
c      parameter         (maxn=256, maxnev=12, maxncv=30, ldv=maxn)
       parameter         (maxn=3000, maxnev=20, maxncv=30, ldv=maxn)
...
c
      Double precision 
     &                  dznrm2 , dlapy2 
      external          dznrm2 , zaxpy , dlapy2, kernel ! I add kernel  
c
c     %-----------------------%
...
c     %--------------------------------------------------% 
c
c       open (24,file='kijr.dat',status='old')
c       open (25,file='kiji.dat',status='old')
c       open (2,file='salida2.out',status='unknown')

        call kernel (Mn,nx)

c       do 101 i =1,nx
c       do 102 j =1,nx
c        kijr(i,j)= dble(kab(i,j))
c        kiji(i,j)= dimag(kab(i,j))
c102    continue
c101    continue
...
c      nx    =4*(nt+1)*ns   !1280  !2304  !1280 = 4*(nt+1)*ns          ! 10 
      n     =nx          !nx*nx 
      nev   = 8   ! 4
      ncv   = 20   !20 
      if ( n .gt. maxn ) then
...
159   continue
      ...
      do 160 k=1,64
      write(30,200)pph(k),dble(F(i,j,k)/F(1,0,1)),
     . dimag(F(i,j,k)/F(1,0,1))
160   continue

158   continue
157   continue
      func= dble(d(nres))-1d0 ! change also in V(ic,here)
      write(2,*)'d(1)',d(1)
      write(2,*)'d(2)',d(2)
      write(2,*)'d(3)',d(3)
      write(2,*)'d(4)',d(4)
