       subroutine dscomplex2(print,mn,pc4,k2r,z1,sigmav,sigmas,
     .                       dsigmav,dsigmas)
       implicit none
       include 'commonf.f'
...
30         if( (( abs(kx2(1)-k2).ge.pmin2).or.
     ...
     .          (flag1.eqv..false.)   )then
       ...
       z(2) = xv(i)*eplus*mh2-(1d0-xv(i))*eminus*mh2  ! +(0,1d0)*1d-10
       z(3) = xp(i)*lambda+(1d0-xp(i))*dsqrt(pmin2) !+(0,1d0)*1d-10

       dz(1)  =  (lambda-dsqrt(pmin2))
       dz(2)  =  (eplus+eminus)*mh2
       dz(3)  = -(lambda-dsqrt(pmin2))

       dkx2(1)=2d0*(z(1)-(0,1d0)*eminus*mh2)*dz(1)
       dkx2(2)=2d0*(-z(2)+(0,1d0)*lambda)*dz(2)
       dkx2(3)=2d0*(z(3)+ (0,1d0)*eplus*mh2)*dz(3)

             
       kx2(1) = z(1)**2-(eminus*mh2)**2-2d0*(0,1d0)*z(1)*eminus*mh2
       kx2(2) = lambda**2-z(2)**2     +2d0*(0,1d0)*z(2)*lambda
       kx2(3) = z(3)**2-(eplus*mh2)**2 +2d0*(0,1d0)*z(3)*eplus*mh2


  
        sumav(1) =1d0*(
     .  wp(i)*BB2(i,1)*dkx2(1)/(kx2(1)-k2)+
     .  wv(i)*BB2(i,2)*dkx2(2)/(kx2(2)-k2)+
     .  wp(i)*BB2(i,3)*dkx2(3)/(kx2(3)-k2))+sumav(1)


       
        sumav(2) =1d0*(
     .  wp(i)*AA2(i,1)*dkx2(1)/(kx2(1)-k2)+
     .  wv(i)*AA2(i,2)*dkx2(2)/(kx2(2)-k2)+
     .  wp(i)*AA2(i,3)*dkx2(3)/(kx2(3)-k2))+sumav(2)
     ...
     .  wp(i)*dkx2(3)/(kx2(3)-k2))+sumav(3)
        ...
        sumav(4) =1d0*(
     .  wp(i)*BB2(i,1)*dkx2(1)/(kx2(1)-k2)**2+
     .  wv(i)*BB2(i,2)*dkx2(2)/(kx2(2)-k2)**2+
     .  wp(i)*BB2(i,3)*dkx2(3)/(kx2(3)-k2)**2)+sumav(4)
 

        sumav(5) =1d0*(
     .  wp(i)*AA2(i,1)*dkx2(1)/(kx2(1)-k2)**2+
     .  wv(i)*AA2(i,2)*dkx2(2)/(kx2(2)-k2)**2+
     .  wp(i)*AA2(i,3)*dkx2(3)/(kx2(3)-k2)**2)+sumav(5)
...
c       Ap2 = Ap*Ap
     ...
     . k2,Bk,Ak
       ...
       write(2,*)'pmin2',pmin2
       write(2,*)'AAinm'
       write(2,*)(AA2(i,1),i=1,np)
       write(2,*)'AAinv'
       write(2,*)(AA2(i,2),i=1,np)
       write(2,*)'AAinp'
       write(2,*)(AA2(i,3),i=1,np)
       write(2,*)'bbinm'
       write(2,*)(bb2(i,1),i=1,np)
       write(2,*)'bbinv'
       write(2,*)(bb2(i,2),i=1,np)
       write(2,*)'bbinp'
       write(2,*)(bb2(i,3),i=1,np)
       write(2,*)'li',li
       write(2,*)'z(i)'
       write(2,*)(z(i),i=1,3)
